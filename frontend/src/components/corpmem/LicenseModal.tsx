...
const LicenseModal: React.FC<LicenseModalProps> = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const data = await fetchJobpostData("00000018");
      setLicenseData(data);
  ...
  const handleCheckboxChange = (
    ...
    const label = option.level3 || option.level2 || option.level1 || "";
        ...
        <form name="chkForm" id="chkForm" method="post" autoComplete="off">
          <div className="tab-navigation">
            <ul className="select_nav">
              {visibleCategories.map((category, index) => (
                <li key={`license-${currentPage}-${index}`}>{category}</li>
              ))}
            </ul>
          </div>
          ...
          <ul className="select_box">
            {visibleCategories.map((category, categoryIndex) => (
              <li key={`licenseBox-${currentPage}-${categoryIndex}`}>
                  ...
                  {groupedData[category]?.map((option) => (
                    ...
                    >
                      ...
                      <label
                        ...
                        className="label"
                      >
                        {option.level3 || option.level2 || option.level1}
                      </label>
                    </li>
                  ))}
                </ul>
