...
const JobApplication: React.FC = () => {
  ...
  const loadApplications = async () => {
    try {
      setIsLoading(true);
      const response = await axiosInstance.get<ApiResponse>('/admin/job-applications', { params: searchParams });
      setApplications(response.data.content);
      setTotalPages(response.data.totalPages);
    } catch (error) {
      console.error('입사지원 목록을 불러오는데 실패했습니다:', error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleBulkDelete = async () => {
    if (selectedIds.length === 0) {
      alert('선택된 입사지원이 없습니다.');
      return;
    }

    if (!window.confirm('선택한 입사지원을 삭제하시겠습니까?')) {
      return;
    }

    try {
      await axiosInstance.delete('/admin/job-applications/bulk', {
        data: { ids: selectedIds }
      });
      alert('선택한 입사지원이 삭제되었습니다.');
      setSelectedIds([]);
      loadApplications();
    } catch (error) {
      console.error('입사지원 삭제에 실패했습니다:', error);
      alert('입사지원 삭제에 실패했습니다.');
    }
  };
                ...
                <div className="search--select select-box">
                  <div className="control">
                    ...
                    >
                      ...
                      <option value="PENDING">미심사</option>
                              ...
                              <td>
                                <div className="btn-area">
                                  <button 
                                    ...
                                    onClick={() => navigate(`/admin/job-application/${application.id}`)}
                                  >
                                    상세
                                  </button>
                                </div>
